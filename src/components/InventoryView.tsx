...
import {
  ...
  Package, X, Check, SlidersHorizontal, RefreshCcw
} from 'lucide-react';
...
export function InventoryView() {
  const { products, updateProduct, removeProduct, clearProducts, filter, setFilter, resetFilter } = useInventoryStore();
  ...
  const startInlineEdit = (id: string, field: string, value: string | number) => {
  ...
  const commitInlineEdit = useCallback(async () => {
    ...
    await updateProduct(id, { [field]: typeof value === 'string' ? (isNaN(Number(value)) ? value : value) : value });
    setInlineEdit(null);
  }, [inlineEdit, updateProduct]);
    ...
    <>
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2.5 border-2 border-slate-200 rounded-xl text-sm text-slate-900
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-600 text-white font-semibold text-sm hover:bg-emerald-700 transition-colors shadow-sm"
          >
        ...
        <div className="flex items-center gap-4 text-sm text-slate-500">
          <span>
            Mostrando <strong className="text-slate-900">{filtered.length}</strong> de {products.length} productos
          </span>
          <span>·</span>
          <span>
            Stock total: <strong className="text-slate-900">{filtered.reduce((s, p) => s + p.stock, 0)}</strong>
          </span>
          <span>·</span>
          <span>
            Valor: <strong className="text-emerald-600">${formatCOP(filtered.reduce((s, p) => s + p.salePrice * p.stock, 0))}</strong>
          </span>
        </div>
                      ...
                      <tr key={product.id} className="hover:bg-slate-50/80 transition-colors group">
                        ...
                        <td className="px-4 py-3">
                          {inlineEdit?.id === product.id && inlineEdit.field === 'salePrice' ? (
                            ...
                            <span
                              className="font-bold text-slate-900 cursor-pointer hover:text-violet-600 transition-colors"
                              onClick={() => startInlineEdit(product.id, 'salePrice', product.salePrice)}
                        ...
                        <td className="px-4 py-3">
                          {inlineEdit?.id === product.id && inlineEdit.field === 'stock' ? (
                            ...
                            <span
                              className={`font-bold cursor-pointer hover:text-violet-600 transition-colors ${
                                product.stock === 0
                                  ? 'text-red-500'
                                  : product.stock <= 3
                                  ? 'text-amber-500'
                        ...
                        {/* Profit */}
                        <td className="px-4 py-3">
                          <span className={`font-semibold text-sm ${profit >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>
                            ${formatCOP(profit)}
                          </span>
                        </td>
